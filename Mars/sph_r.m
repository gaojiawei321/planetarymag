function [B,ddBr,ddBt,ddBp]=sph_r(g,h,rplanet,scr, sct, scp)
...
% Calculate Legendre polynomials and derivatives recursively
    ...
    %        dP(m+1,n+1)=dP2;
          ...
          ddBr(m+1,n+1)=rn(n)*(n+1)*((g(m+1,n+1)*cmp(m+1) + h(m+1,n+1)*smp(m+1) ))*P2*S1(m+1,n+1);
          ddBt(m+1,n+1)=rn(n)*((g(m+1,n+1)*cmp(m+1) + h(m+1,n+1)*smp(m+1)))*dP2 * S1(m+1,n+1);
          ddBp(m+1,n+1)=rn(n)*(m*(-g(m+1,n+1)*smp(m+1) + h(m+1,n+1)*cmp(m+1)))* P2 * S1(m+1,n+1);
